...
function tarjetaPropiedad(prop) {
    ...
    if (prop.smoke) {
        propiedad_html += `<p class= "text-success"><i class= "fas fa-smoking"}></i>Permitido fumar</p>`;
    } else {
        propiedad_html += `<p class= "text-danger"><i class= "fas fa-smoking-ban"}></i>No se permite fumar</p>`;
    };

    if (prop.pets) {
        propiedad_html += `<p class= "text-success"><i class= "fa-solid fa-paw"}></i>Mascotas permitidas</p>`;
    } else {
        propiedad_html += `<p class= "text-danger"><i class= "fa-solid fa-ban"}></i>No se permiten mascotas</p>`;
    };
    ...
    return propiedad_html;
};
let enVentaDestacada_html = ``;
let enAlquilerDestacada_html = ``;

let enVentaTotal_html = ``;
let enAlquilerTotal_html = ``;

for (let i = 0; i < 3; i++) {
    enVentaDestacada_html += tarjetaPropiedad(propiedadesEnVenta[i]);
    enAlquilerDestacada_html += tarjetaPropiedad(propiedadesEnAlquiler[i]);    
};

for (let prop of propiedadesEnVenta) {
    enVentaTotal_html += tarjetaPropiedad(prop); 
};

for (let propi of propiedadesEnAlquiler) {
